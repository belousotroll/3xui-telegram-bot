...
def load_approval_requests() -> List[Dict[str, Any]]:
    ...
    if not os.path.exists(APPROVAL_REQUESTS_FILE):
        ...
        try:
            ...
            logger.warning(f"{APPROVAL_REQUESTS_FILE} not found, created new file with empty list")
        except OSError as e:
    ...
    if not os.path.exists(APPROVED_USERS_FILE):
        ...
        try:
            with open(APPROVED_USERS_FILE, 'w', encoding='utf-8') as f:
                json.dump([], f, ensure_ascii=False, indent=4)
            logger.warning(f"{APPROVED_USERS_FILE} not found, created new file with empty list")
        except OSError as e:
...
def load_admins() -> List[int]:
    ...
    при отсутствии создаёт пустой файл и возвращает пустой список.
    """
    if not _validate_path(ADMIN_IDS_FILE, 'Admin IDs file'):
        return []

    if not os.path.exists(ADMIN_IDS_FILE):
        _ensure_dir(ADMIN_IDS_FILE)
        try:
            with open(ADMIN_IDS_FILE, 'w', encoding='utf-8'):
                pass
            logger.warning(f"{ADMIN_IDS_FILE} not found, created new file with no admins")
        except OSError as e:
    ...
    if not admins:
        logger.warning(f"No valid admin IDs found in {ADMIN_IDS_FILE}")
    return admins
